...
public class HexUtils {  
    ...
    public static final String bytesToHexString(byte[] bArray,int len) {
        StringBuffer sb = new StringBuffer(bArray.length);
        String sTemp;
        for (int i = 0; i < Math.min(bArray.length,len); i++) {
            sTemp = Integer.toHexString(0xFF & bArray[i]);
            if (sTemp.length() < 2)
                sb.append(0);
            sb.append(sTemp.toUpperCase());
     ...
     * @return HexString
     */
    public static final String bytesToHexString(byte[] bArray,int beg,int len) {
        StringBuffer sb = new StringBuffer(bArray.length);
        String sTemp;
        for (int i = beg; i < Math.min(bArray.length,beg+len); i++) {
            sTemp = Integer.toHexString(0xFF & bArray[i]);
            if (sTemp.length() < 2)
                sb.append(0);
            sb.append(sTemp.toUpperCase());
    ...
    /**
     * short 转 byte数组， 2个字节或者4个字节 short to byte[], length=2
     *
     * @param shortValue
     * @return
     */
    public static byte[] shortToByteArray(short shortValue) {
        ...
        result = new byte[2];
        result[0] = (byte) (shortValue  >> 8);
        result[1] = (byte) (shortValue & 0xFF);
