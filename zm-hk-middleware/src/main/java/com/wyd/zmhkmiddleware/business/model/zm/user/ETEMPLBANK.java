
package com.wyd.zmhkmiddleware.business.model.zm.user;

import com.google.gson.annotations.SerializedName;

@SuppressWarnings("unused")
public class ETEMPLBANK {

    @SerializedName("BANK_ID")
    private String bANKID;
    @SerializedName("BANKL")
    private String bANKL;
    @SerializedName("KOINH")
    private String kOINH;
    @SerializedName("LAND1")
    private String lAND1;
    @SerializedName("ZATTR1_BK")
    private String zATTR1BK;
    @SerializedName("ZATTR2_BK")
    private String zATTR2BK;
    @SerializedName("ZATTR3_BK")
    private String zATTR3BK;
    @SerializedName("ZATTR4_BK")
    private String zATTR4BK;
    @SerializedName("ZATTR5_BK")
    private String zATTR5BK;
    @SerializedName("ZATTR6_BK")
    private String zATTR6BK;
    @SerializedName("ZBANKA")
    private String zBANKA;
    @SerializedName("ZBANKN")
    private String zBANKN;
    @SerializedName("ZDEL_BK")
    private String zDELBK;
    @SerializedName("ZHREMPL")
    private String zHREMPL;
    @SerializedName("ZMAIN")
    private String zMAIN;
    @SerializedName("ZPOSTTY")
    private String zPOSTTY;

    public String getBANKID() {
        return bANKID;
    }

    public void setBANKID(String bANKID) {
        this.bANKID = bANKID;
    }

    public String getBANKL() {
        return bANKL;
    }

    public void setBANKL(String bANKL) {
        this.bANKL = bANKL;
    }

    public String getKOINH() {
        return kOINH;
    }

    public void setKOINH(String kOINH) {
        this.kOINH = kOINH;
    }

    public String getLAND1() {
        return lAND1;
    }

    public void setLAND1(String lAND1) {
        this.lAND1 = lAND1;
    }

    public String getZATTR1BK() {
        return zATTR1BK;
    }

    public void setZATTR1BK(String zATTR1BK) {
        this.zATTR1BK = zATTR1BK;
    }

    public String getZATTR2BK() {
        return zATTR2BK;
    }

    public void setZATTR2BK(String zATTR2BK) {
        this.zATTR2BK = zATTR2BK;
    }

    public String getZATTR3BK() {
        return zATTR3BK;
    }

    public void setZATTR3BK(String zATTR3BK) {
        this.zATTR3BK = zATTR3BK;
    }

    public String getZATTR4BK() {
        return zATTR4BK;
    }

    public void setZATTR4BK(String zATTR4BK) {
        this.zATTR4BK = zATTR4BK;
    }

    public String getZATTR5BK() {
        return zATTR5BK;
    }

    public void setZATTR5BK(String zATTR5BK) {
        this.zATTR5BK = zATTR5BK;
    }

    public String getZATTR6BK() {
        return zATTR6BK;
    }

    public void setZATTR6BK(String zATTR6BK) {
        this.zATTR6BK = zATTR6BK;
    }

    public String getZBANKA() {
        return zBANKA;
    }

    public void setZBANKA(String zBANKA) {
        this.zBANKA = zBANKA;
    }

    public String getZBANKN() {
        return zBANKN;
    }

    public void setZBANKN(String zBANKN) {
        this.zBANKN = zBANKN;
    }

    public String getZDELBK() {
        return zDELBK;
    }

    public void setZDELBK(String zDELBK) {
        this.zDELBK = zDELBK;
    }

    public String getZHREMPL() {
        return zHREMPL;
    }

    public void setZHREMPL(String zHREMPL) {
        this.zHREMPL = zHREMPL;
    }

    public String getZMAIN() {
        return zMAIN;
    }

    public void setZMAIN(String zMAIN) {
        this.zMAIN = zMAIN;
    }

    public String getZPOSTTY() {
        return zPOSTTY;
    }

    public void setZPOSTTY(String zPOSTTY) {
        this.zPOSTTY = zPOSTTY;
    }

}
