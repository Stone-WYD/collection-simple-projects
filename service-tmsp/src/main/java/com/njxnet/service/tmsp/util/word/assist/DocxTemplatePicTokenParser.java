...
public class DocxTemplatePicTokenParser {
    private final String openToken;
    private final String closeToken;
    private final DocxTemplatePicTokenHandler handler;

    public DocxTemplatePicTokenParser(String openToken, String closeToken, DocxTemplatePicTokenHandler handler) {
        this.openToken = openToken;
        this.closeToken = closeToken;
        this.handler = handler;
    }

    public String parse(String text,  CustomXWPFDocument doc, XWPFRun run) throws IOException, InvalidFormatException {
                ...
                for(StringBuilder expression = null; start > -1; start = text.indexOf(this.openToken, offset)) {
                    if (start > 0 && src[start - 1] == '\\') {
                        ...
                        if (end == -1) {
                            ...
                            builder.append(this.handler.handleToken(expression.toString(), doc, run));
                            offset = end + this.closeToken.length();
                        }
                    }
                }
